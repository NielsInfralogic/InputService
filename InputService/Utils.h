...
//	int		IssueMessage(CDatabaseManager &DB, CJobAttributes &Job, int nStatus, CString &sErrorMessage);
	int IssueMessage(CDatabaseManager &DB, CJobAttributes &Job, int nStatus, CString sTargetFolder, int nType, CString &sErrorMessage);


	DWORD GenerateGUID(int nSpooler, int nTick);

	CString FormNameChannel(CJobAttributes &job, CHANNELGROUPSTRUCT &channelGroup, int nDaysToAdvance);
	CString  FormName(CString sOrgMask, CString sDateFormat, CString sTimeFormat, CJobAttributes &job, int bAbbr, int nDaysToAdvance);

	CString GetRawMaskEx(CString sOrgMask, CString &sDateFormat);
	int		GetWeekNumberFromDate(CTime tDate);
	CTime	GetDateFromWeekNumber(int nYear, int nWeekNumber, int nDayOfWeek);
	int		PageName2PageNumber(CString sPageName);

	BOOL  Decompose(CString sJobName, INPUTPROCESSSTRUCT *pInput, CJobAttributes *pJob,
