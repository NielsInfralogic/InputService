...
TCHAR	g_szServiceName[MAX_PATH] = _T("InputService");
TCHAR	g_szServiceDescription[MAX_PATH] = _T("PDFhub File Input Service");
...
BOOL InstallService()
{
	...
	keyClasses.Create(HKEY_LOCAL_MACHINE, _T("Software\\InfraLogic\\InputService"));
	...
	keyClasses.SetKeyValue(_T("Parameters"), _T("c:\\InputService"), _T("LogFileFolder"));
	...
	keyClasses.SetKeyValue(_T("Parameters"), _T("c:\\InputService\\Config.ini"), _T("ConfigFile"));
...
void WINAPI ServiceMain(DWORD argc, LPTSTR *argv)
{
	...
	if (g_ServiceStatusHandle == (SERVICE_STATUS_HANDLE)0) {
		LogEvent(EVENTLOG_ERROR_TYPE, IDS_ERRSERVICECTRL, NULL, NULL, NULL);
		bOkToRun = FALSE;
	}
	...
	strcpy(szEventStr, "InputService");
	...
	if (g_prefs.LoadPreferencesFromRegistry() == FALSE)
		g_util.Logprintf("CANNOT READ Registry: Software\\InfraLogic\\InputService\\Parameters..\r\n");
	else {
		g_util.Logprintf("INFO: Registry read: Software\\InfraLogic\\InputService\\Parameters..\r\n");
	...
	CString sIniFile = _T("");
	g_prefs.m_setupXMLpath = _T("");
	g_prefs.m_setupEnableXMLpath = _T("");
	...
	if (dbmaint.LoadConfigIniFile(g_prefs.m_instancenumber, sIniFile, g_prefs.m_setupXMLpath, g_prefs.m_setupEnableXMLpath, sErrorMessage))
...
HINSTANCE hWeekToDate = NULL;
DATEOFWEEK pDateOfWeek;

int LoadWeekToDateDll()
{
	CUtils util;

	if ((hWeekToDate = ::LoadLibrary(_T("WeekToDate.dll"))) == NULL) {
		AfxMessageBox(_T("Error loading WeekToDate.dll"), MB_ICONSTOP);
		return FALSE;
	}
	else {
		if ((pDateOfWeek = DATEOFWEEK(GetProcAddress(hWeekToDate, "DateOfWeek"))) == NULL) {
			AfxMessageBox(_T("Error finding function names in WeekToDate.dll"), MB_ICONSTOP);
			return FALSE;
		}
	}

	util.Logprintf("INFO: WeekToDate.dll loaded");
